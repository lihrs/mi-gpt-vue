...
app.post('/api/config', async (req, res) => {
  try {
    ...
    // 构建 .env.production 内容
    const envLines = [];

    console.log(2222)
    // 如果有选中的 AI 服务配置，添加到 .env.production 文件
    ...
    // 如果使用自定义 TTS，添加 TTS 配置到 .env.production 文件
    ...
    if (envLines.length > 0) {
      ...
      await fs.promises.writeFile('.env.production', envContent, 'utf8');
      console.log('.env.production 文件已更新');
    }
...
app.get('/api/config/env', async (req, res) => {
  try {
    ...
    console.error('读取 .env.production 文件失败:', error);
    // 发生错误时返回空配置而不是错误状态
    res.json({
      OPENAI_API_KEY: '',
      OPENAI_MODEL: '',
...
TTS_SWITCH_KEYWORDS=${TTS_SWITCH_KEYWORDS || ''}`;
    ...
    res.json({
      success: true,
      message: '.env.production 文件已更新'
    });
  } catch (error) {
    console.error('更新 .env.production 文件失败:', error);
    res.status(500).json({
      success: false,
      error: '更新 .env.production 文件失败: ' + error.message
    });
  }
});
