...
// node_modules/.pnpm/tsup@8.3.5_tsx@4.19.2_typescript@5.7.3/node_modules/tsup/assets/cjs_shims.js
...
var readFile2 = (filePath, options) => {
  const dirname2 = path__namespace.dirname(filePath);
  if (!fs__namespace.existsSync(dirname2)) {
    return undefined;
  }
  return new Promise((resolve2) => {
    fs__namespace.readFile(filePath, options, (err, data) => {
...
var writeFile2 = (filePath, data, options) => {
  const dirname2 = path__namespace.dirname(filePath);
  if (!fs__namespace.existsSync(dirname2)) {
    fs__namespace.mkdirSync(dirname2, { recursive: true });
  }
  return new Promise((resolve2) => {
    {
      fs__namespace.writeFile(filePath, data, options, (err) => {
...
function signNonce(ssecurity, nonce) {
  let m = crypto__namespace.createHash("sha256");
...
var _http = axios__default.default.create(_baseConfig);
...
function decodeMiIOT(ssecurity, nonce, data, gzip) {
  ...
  if (gzip) {
    try {
      decrypted = pako__namespace.ungzip(decrypted, { to: "string" });
    } catch (err) {
      error = err;
    }
  }
...
process.cwd();
var exists = (filePath) => fs2__default.default.existsSync(filePath);
var readFile3 = (filePath, options) => {
  const dirname2 = path__namespace.default.dirname(filePath);
  if (!fs2__default.default.existsSync(dirname2)) {
    return undefined;
  }
  return new Promise((resolve2) => {
    fs2__default.default.readFile(filePath, options, (err, data) => {
      resolve2(err ? undefined : data);
    });
  });
};
var writeFile3 = (filePath, data, options) => {
  const dirname2 = path__namespace.default.dirname(filePath);
  if (!fs2__default.default.existsSync(dirname2)) {
    fs2__default.default.mkdirSync(dirname2, { recursive: true });
  }
  return new Promise((resolve2) => {
    {
      fs2__default.default.writeFile(filePath, data, options, (err) => {
        resolve2(err ? false : true);
      });
    }
  });
...
var deleteFile = (filePath) => {
  try {
    fs2__default.default.rmSync(filePath);
    return true;
  } catch {
    return false;
  }
